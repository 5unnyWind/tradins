...
const InputSchema = z.object({
  ...
  eventLimit: z.coerce.number().int().min(1).max(40).optional().default(16),
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Invalid pro impact query";
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Unknown pro impact error";
    return NextResponse.json(
      {
        ok: false,
        error: message,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Invalid pro impact payload";
    return NextResponse.json(
      {
        ok: false,
        error: message,
